...
def _setupSSHDImpl(public_key, tunnel, ngrok_token, ngrok_region, mount_gdrive_to, mount_gdrive_from, is_VNC):
  ...
  subprocess.run(["chpasswd"], input = f"{user_name}:{root_password}", universal_newlines = True)
  ...
  if tunnel == "ngrok":
    ...
    m = re.match(r"tcp://(.+):(\d+)", ssh_tunnel.public_url)
    hostname = m.group(1)
    port = m.group(2)
  ...
  if is_VNC:
    ...
    msg += "Command to connect to the ssh server as root:\n"
    msg += "✂️"*24 + "\n"
